...
-- Dual-context intensional PCF

module Dual.IPCF where
...
rename ρ₁ ρ₂ (succ M)    = succ (rename ρ₁ ρ₂ M)
rename ρ₁ ρ₂ (ifz L then M else N) =
  ifz rename ρ₁ ρ₂ L then rename ρ₁ ρ₂ M else rename (S_ ∘ ρ₁) ρ₂ M
...
(proj₂ L) ⟪ σ₁ ︔ σ₂ ⟫ = proj₂ (L ⟪ σ₁ ︔ σ₂ ⟫)
zero      ⟪ σ₁ ︔ σ₂ ⟫ = zero
(succ M)  ⟪ σ₁ ︔ σ₂ ⟫ = succ (M ⟪ σ₁ ︔ σ₂ ⟫)
(ifz L then M else N) ⟪ σ₁ ︔ σ₂ ⟫ =
  ifz L ⟪ σ₁ ︔ σ₂ ⟫ then M ⟪ σ₁ ︔ σ₂ ⟫ else (N ⟪ exts σ₁ ︔ σ₂ ⟫ )
⌜ M ⌝     ⟪ σ₁ ︔ σ₂ ⟫ = ⌜ M ⟪ `_ ︔ σ₂ ⟫ ⌝
...
data _︔_⊢_-→_ (Δ Γ : Cxt) : (M N : Δ ︔ Γ ⊢ A) → Set where
  ...
  β-ifz-zero
    : Δ ︔ Γ ⊢ ifz zero then M else N -→ M
  β-ifz-succ
    : Δ ︔ Γ ⊢ ifz (succ L) then M else N -→ N [ L ]
  ...
  ξ-ifz₁
    : Δ ︔ Γ ⊢ L -→ L′
    → Δ ︔ Γ ⊢ ifz L then M else N -→ ifz L′ then M else N
...
progress (succ M)    = done succ
progress (ifz L then M else N) with progress L
... | step L→L′ = step (ξ-ifz₁ L→L′)
... | done zero = step β-ifz-zero
... | done succ = step β-ifz-succ
progress (mfix M)    = step β-mfix
progress ⌜ M ⌝       = done ⌜ M ⌝
