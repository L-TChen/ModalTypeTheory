...
infix   5  ƛ_⇒_ mlet_≔_`in_ mfix_⇒_ _⇒ _⇐_
infixl  7  _·_
infix   9  `_
...
inherit Δ Γ (mfix x ⇒ M) (□ A)   with inherit Δ (Δ , x ⦂ □ A) M A
... | no ¬⊢M = no λ { (⊢mfix ⊢M) → ¬⊢M ⊢M }
... | yes ⊢M = yes (⊢mfix ⊢M)
inherit Δ Γ (mfix x ⇒ M) ⊤̇       = no λ ()
inherit Δ Γ (mfix x ⇒ M) (A ×̇ B) = no λ () 
inherit Δ Γ (mfix x ⇒ M) (A →̇ B) = no λ ()
